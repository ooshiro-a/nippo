...
- 「今月の進捗」の中に **個人末見 / 営業所末見** の2行を出す。元は
  「個人末見額達成率」「営業所末見額達成率」「促進受注額達成率」の達成率カード3枚だったが、
  2026-09-05 に廃止して行に寄せた（`renderPlanCard` / `renderPaceLine` も削除）
- **数字は `officeDaily`（日計表の取込）から拾う。** 入力タブの「末見額」カードの手入力ではない。
  個人分は日計表「一枚目 」の**29行目＝所員eの売上行**で、`cols.sales.salesForecast`（J列）
  として既に取り込まれている。**parserConfig.js / GAS の改修は不要**
- 末見通しは `_officeSalesForecast()`（実績＋A案件）で出す。保存値の `salesForecast` を
  そのまま読まないこと。片方だけ計算を変えると営業所タブと数字が食い違う
- **自分の行は `memberName` で探す**（`MY_MEMBER_NAME`）。`memberId === 'e'` の決め打ちだと
  所員の増減で行がずれたとき**黙って他の人の数字が出る**。名前→id→手入力値の3段構えにしてある
- 週次に切り替えても同じ値を出す。末見額は「月末にいくらになりそうか」の見込みなので
  週で割っても意味が無い（あきぼー判断）
- 元の達成率カードにあった**不足額はペース行に併記**して引き継いだ
- **ラベルは4〜5文字まで。** `.weekly-gauge-label` の1つ目の span は `flex:1` で縮むため、
  金額が8桁×2 の行ではラベルに60〜76pxしか残らず、6文字だと375pxで2行に折り返す。
  この2行だけ金額を11pxにして5文字（営業所末見）が入るようにしてある。
  営業所タブの「末見通し」「売上 実績」が短いのも同じ理由
- 手入力の末見額（`personalUnsettled` / `officeUnsettled`）と KGI設定の計画額は**残してある**。
  取込が無い日の逃げ道として使う
...
- **単位は「台」**。継続も新規保守も**点検も**台数で数える（点検は 2026-09-08 に件→台へ変更）。
  促進・エアコン洗浄などは回数なので「件」のまま。
  個人の `KGI_FIELDS` も台にそろえてある。**個人と営業所は同じ数字を2箇所から見ているだけ**なので、
  片方だけ単位を変えると同じ値が「30件」「30台」に割れる。根拠は数字の一致：
  保守継続は 個人 当月23/次月14/次々月7 ＝ 大城章裕の行、
  点検は 個人の2026-09目標30 ＝ 大城章裕の点検計画30
...
### 営業所の期間集計（最重要・2026-09-08 Step25）

営業所の値は月内累計なので、**日次行をそのまま足してはいけない**。
足すと取り込み回数だけ膨らむ。2026年の点検実績が 2,186件 → 4,910件（2.2倍）に化けていた。
2025年は月末1行しか無いため足しても正しく見えており、**バグが1年間気づかれなかった**。

- **月をまたぐ集計は「各月の最終行だけ取って合計」**。`_sumOfficeByMonth()`（app.js）に集約した。
  計画（`*Plan`）も月ごとの定数なので実績と同じルールでよい
- **末見通しは合計後に `_officeSalesForecast()` で計算し直す。** 保存値を足さない
- **年次・四半期は `_sumOfficeByMonth()` を通すこと。** `latest` を取ると
  期間の最終月の値しか出ない（実際「2026年（年末実績）点検実績 0件」と出ていた）
- **期間比較は月数をそろえる**（`_alignOfficeMonths()`）。2026年1〜9月 と 2025年1〜12月 を
  そのまま比べると、月数の差が増減率に化ける（点検実績 -8.8% が -31.0% に見える）
- **週次比較は増分**（`_officeWeekDelta()`）。週末の累計 − 週初直前の同月内最終行。
  計画は月間計画÷3（進捗タブの週次ゲージと同じ扱い）
- **日次は累計のまま**。ただし同じ日に複数行あるので最終1行に寄せる（`_officeDayLatest()`）。
  実際 `2026-05-29` は2行あり、足すと全項目が2倍になっていた
- GAS の `generateOfficeReportImpl()` も同じ。yearly / quarterly だけ月末値合計にしてある。
  daily / weekly / monthly は単月内なので `getLatest()` のままで正しい
- 比較カードには**集計方法を1行出す**（`basisNote`）。ビューごとに足し方が違うので、
  出さないと履歴カードの数字と食い違って見えたときに判断できない

**残っているズレ（未対応）**: 履歴の週カードは月曜起点、週次比較は `Math.ceil(日/7)`（第N週の
ドロップダウンに合わせている）で区切りが違う。同じ「第3週」でも範囲が違う。

**CSVは日次明細のまま**（各行＝その日の月内累計）。列を単純合計しても意味は無い。
数字の確認は履歴タブの期間比較を見ること。

### 点検実行数（手入力）— 2026-09-08 Step26

日計表の点検実績（`inspectionActual`）は**反映が遅れる**。2026年9月は所員全員・全日 0 のまま
（計画は 29+32+26+25+30＝142 と正しく入っている）。7月も `07-02:8 → 07-08:82 → 07-31:290` と
飛び飛びにしか埋まらない。月の途中では進捗として使えない。

- **`inspectionManual`（点検実行数）は別フィールド。** `inspectionActual` は日計表の値のまま。
  月末には日計表も確定するので、突き合わせに使えるよう消していない
- **入力は取込タブ**（日計表取込と売上計画取込の間）。日計表を取り込むついでに入れて
  忘れないようにするため。進捗タブではない
- **入れる数字はその月の累計台数。** 営業所DBが月内累計なので、これで週次の増分
  （`wn()`）も履歴の集計もそのまま乗る
- **保存は新しい行を作らず、その月の最新の office 行を丸ごと送り直す。**
  新しい行を作ると `renderManagementDashboard(officeRows[0], ...)` が最新1行しか見ないため、
  売上・保守・継続が全部ゼロで表示される。`saveOfficeDailyImpl` は行を丸ごと置き換えるので
  全列を送り返すこと
- **`_fillOfficeManualForward()` を取得直後に必ず通す。** 手入力後に日計表を取り込むと
  新しい日付の行ができ、その行の `inspectionManual` は空。埋めないと入れた値が画面から消える。
  呼ぶのは `ensureOfficeHistData()` と `refreshManagement()` と `loadInspectionManual()`
- 表示は**進捗タブ・履歴タブとも「実行数がメイン、日計表の実績はサブ行」**。
  仕組みは売上の末見通しと同じ（`mainCalc`/`mainLabel`/`subLabel`、進捗は `calc`/`subKey`/`subLabel`）
- **総合達成率（`_calcOfficeKpiRate`）は日計表の実績のまま。触らないこと。**
  売上を末見通しベースにしたとき（Step24）と同じ扱いで、意図的に2系統に分けてある
- **警告バナー（`_renderKpiAlerts`）は実行数で判定する。** 実績で判定すると当月ずっと0なので
  毎月かならず「点検が40%未満」と鳴って警告として機能しない
- GAS の `OFFICE_DAILY_COLS` 末尾に `inspectionManual` を追加済み。
  `saveOfficeDailyImpl` にも `_ensureSheetColumns_` を入れた（POSTだけの実行では
  列が確保されず `setValues` が範囲外で落ちるため）

### 営業所 履歴タブの単月比較（2026-09-08 Step26）

- 月次ビューは**比較ONのときだけ**期間の選び方が変わる。
  OFF＝年を選んで全月カード（従来）／ON＝年月を選んで単月 vs 単月、カードは1枚
- 既定は**前年同月**（`_officePrevYearMonth()`）
- 単月どうしなので `_alignOfficeMonths()` は呼ばない。`_sumOfficeByMonth()` にその月の行だけ渡す
- 比較ON/OFFで期間の選び方が変わるのは日次ビューで既に使っている流儀
...
| Step24b | 個人 進捗タブに末見額2行を統合（達成率カード3枚を廃止） | 完了 |
| Step25 | 営業所の期間集計を月末値合計に修正 + 比較期の表示改善 + 進捗タブの印刷/出力設定 | 完了 |
| Step26 | 履歴タブの単月比較（対前年同月）+ 点検実行数の手入力 + 点検の単位を台に | 完了 |
