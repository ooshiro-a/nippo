...
- **ゲージ・%・前期比は「末見通し」で出す**（`末見通し = 売上実績 + A案件`）。
  実績はサブ行に回す。計画にどこまで届きそうかを見たいため（2026-09-05 あきぼー指示）
- 仕組みは `_OFFICE_KPI_DEFS` の売上に付けた **`mainCalc` / `mainLabel` / `subLabel`**。
  他の項目には付けないので今までどおり実績で動く
- **`_calcOfficeKpiRate` は実績ベースのまま。触らないこと。**
  こちらは総合達成率・順調/要注意バッジ・レポート出力の月別内訳が使う。
  あきぼー判断で「総合達成率は実績のまま（21%）」としたので、意図的に2系統に分けてある
- `label` も `'売上'` のまま残す。月別内訳と出力設定のチェックボックスが `d.label` を使うため。
  見出しだけ `mainLabel` で差し替える
- 出力設定の `showForecast` は**印刷時に**サブ行（`.ohist-forecast-row`）を隠す設定。
  サブ行の中身が末見通しから実績に変わったので、ラベルも「売上の実績・対計画」にした
...
- 本番は **`https://ooshiro-a.github.io/nippo/`**。ブックマークもこれにする
- GAS の `/exec` を直接開くと、`doGet` のリダイレクトが **Googleのページの内側で起きる**。
  アドレスバーは `script.google.com` のまま、その中にアプリが埋め込まれた状態になる
- 埋め込み状態だと2つ困ることが起きる
  - **タブのアイコンが Apps Script のもの**になる。外側は Google のページなので変えられない
  - キャッシュや localStorage が通常のタブと別扱いになり、古い版が出たり、
    表示設定・並び順が引き継がれなかったりする
- 「アイコンが違う」「直したはずの表示が古い」と言われたら**まずURLを確認する**。
  2026-09-05 に実際これが起きた（単位を台に直したのに件のままだった）
...
| Step24 | 営業所 履歴タブの売上を末見通しベースのゲージに変更 | 完了 |
