 ...
 * デプロイ設定:
 *   「次のユーザーとして実行」→「自分」
 *   「アクセスできるユーザー」→「全員」（GitHub Pages 版の fetch() から ?action=... を呼ぶため必須）
 *
 *   ※この公開設定はURLを知る誰もが読み書きできることを意味する。
 *     2026-09-05 にリスクを認識したうえで当面現状維持と判断済み。
 *     経緯と再検討条件は CLAUDE.md「受容済みリスク」の節を参照。
 *
 * 配信方式:
